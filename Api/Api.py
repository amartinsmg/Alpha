...
def roots_vertex():
    ...
    if delta > 0:
        ...
        x1Num = float(y[0])
        x2Num = float(y[1])
        x1NumF = str(x1Num) if len(str(x1Num)) <= len(format(
            x1Num, '.5f')) else format(x1Num, '.5f')
        x2NumF = str(x2Num) if len(str(x2Num)) <= len(format(
            x2Num, '.5f')) else format(x2Num, '.5f')
        x1 = x1Sym if x1Num % 1 == 0 else ('{} (Decimal {})').format(
            x1Sym, x1NumF)
        x2 = x2Sym if x2Num % 1 == 0 else ('{} (Decimal {})').format(
            x2Sym, x2NumF)
        realRoots = True
    elif delta == 0:
        xSym = str(y[0])
        xNum = float(y[0])
        xNumF = str(xNum) if len(str(xNum)) <= len(format(
            xNum, '.5f')) else format(xNum, '.5f')
        x1 = x2 = xSym if xNum % 1 == 0 else ('{} (Decimal {})').format(
            xSym, xNumF)
        realRoots = True
    else:
        x1 = x2 = ''
